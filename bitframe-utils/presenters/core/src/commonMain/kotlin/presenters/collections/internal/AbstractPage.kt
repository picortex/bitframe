...
abstract class AbstractPage<out T> : Page<T> {
    ...
    override fun <R> map(transformer: (item: T) -> R) = Page(
        items = items.map { transformer(it.item) }.toInteroperableList(), capacity, no
    )

    override fun <R> mapIndexed(transformer: (index: Int, item: T) -> R) = Page(
        items = items.mapIndexed { index, it -> transformer(index, it.item) }.toInteroperableList(), capacity, no
    )
