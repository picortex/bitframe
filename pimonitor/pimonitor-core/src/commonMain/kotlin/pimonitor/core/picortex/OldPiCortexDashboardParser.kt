...
internal class OldPiCortexDashboardParser(val mapper: Mapper) {
    ...
    fun parseProductsSoldChart(json: String) = BarChart(
        title = "Products sold", description = "Products sold this month", entries = parseBarChartEntriesOf(
            type = PRODUCTS_SOLD, from = json
        )
    )

    fun parseJobsPerClientChart(json: String) = BarChart(
        title = "Jobs per client", description = "Jobs fulfilled in the last month", entries = parseBarChartEntriesOf(
            type = JOBS_PER_CLIENT, from = json
        )
    )

    fun parseHoursPerEmployee(json: String) = BarChart(
        title = "Hours per employee", description = "Hours worked in the last month", entries = parseBarChartEntriesOf(
