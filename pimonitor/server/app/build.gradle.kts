...
kotlin {
    ...
    sourceSets {
        val main by getting {
            dependencies {
                api(projects.pimonitorMonitorSdkServerCore)
...
val createDockerfile by tasks.creating(Dockerfile::class) {
    ...
    runCommand("mkdir /app /app/public /app/configs")
    destFile.set(file("build/binaries/Dockerfile"))
    copyFile("./release", "/app")
    copyFile("./config", "/app/config")
    doFirst {
        copy {
            from("config")
            into("build/binaries/config")
        }
    }
    workingDir("/app")
    exposePort(8080)
    defaultCommand("./bin/pimonitor-server-app", "/app/public")
}
...
fun DockerComposeFileTask.configure(port: Int, config: String) {
    version(3.8)
    service("database") {
        image("mongo:5.0.6")
        set(
    ...
    service("server-app") {
        ...
        set(
            "environment", mapOf(
                "PIMONITOR_ROOT" to "/app",
                "PIMONITOR_CONFIG" to config
            )
        )
        ports(port to 8080)
    }
...
val createDockerComposeStagingFile by tasks.creating(DockerComposeFileTask::class) {
    ...
    configure(port = 90, config = "staging")
}
