...
import pimonitor.core.configs.piCortexDashboard
...
class BusinessOverviewServiceDaod(
    ...
    private val picortex by lazy { config.piCortexDashboard }
    ...
    override fun load(rb: RequestBody.Authorized<LoadInfoParsedParams>): Later<MonitoredBusinessOverview> = config.scope.later {
        ...
        var overview: MonitoredBusinessOverview = MonitoredBusinessOverview.NotIntegrated(business)
        if (operational != DASHBOARD_OPERATIONAL.NONE) {
            overview += operationalOverview(business, params)
        }
        if (financial != DASHBOARD_FINANCIAL.NONE) {
            overview += financialOverview(business, params)
        }
        overview
    }

    private suspend fun operationalOverview(business: MonitoredBusinessBasicInfo, params: LoadInfoParsedParams): MonitoredBusinessOverview.OperationalOnly = when (business.operationalBoard) {
        DASHBOARD_OPERATIONAL.PICORTEX -> {
            ...
            val dashboard = picortex.technicalDifferenceDashboardOf(credentials, params.start, params.end).await()
            val revenueVsExpensesChart = dashboard.charts.first { it.title.contentEquals("Revenue vs Expenses", ignoreCase = true) }
            val otherChart = (dashboard.charts - revenueVsExpensesChart).first()
